...
def face_detect(directory):
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".jpg") or filename.endswith(".jpeg") or filename.endswith(".png"): 
            ...
            face_cascade = cv2.CascadeClassifier('pre-trained_data/haarcascades/haarcascade_frontalface_alt2.xml')
...
def face_recog(directory,filename):
    ...
    finale_image = cv2.resize(gray,(600,500),interpolation=cv2.INTER_AREA)
    faces = face_cascade.detectMultiScale(finale_image, scaleFactor=1.2, minNeighbors=5)
    # cv2.imshow('image1',image1)
    ######################################3
    for (x, y, w, h) in faces:
        roi = finale_image[y:y+h, x:x+w]
        ...
        if conf>60:
            if faces is ():
                ...
                dir_path = os.path.join(moveToPath,lables[id_])
                # print("dir path:",dir_path)
                os.mkdir(dir_path)
                moveToPathFile = os.path.join(dir_path, filename) #destination folder path for the specific image
                # print("file path:",moveToPathFile)
